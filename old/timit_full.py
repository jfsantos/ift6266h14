# This file is based on NLTK's TIMIT Corpus Reader, but adapted to
# read the whole dataset (and use the original filenames/structure).
#
...
"""
Read tokens, phonemes and audio data from the Full TIMIT Corpus.
...
class TimitFullCorpusReader(CorpusReader):
    ...
    def fileids(self, filetype=None):
        ...
        if filetype is None:
            ...
            return ['DOC/TIMITDIC.TXT', 'DOC/SPKRINFO.TXT']
        else:
    ...
    def transcription_dict(self):
        ...
        """
        _transcriptions = {}
        for line in self.open('DOC/TIMITDIC.TXT'):
    ...
    def spkrinfo(self, speaker):
        ...
        if self._speakerinfo is None:
            self._speakerinfo = {}
            for line in self.open('DOC/SPKRINFO.TXT'):
                if not line.strip() or line[0] == ';': continue
                rec = line.strip().split(None, 9)
                key = "DR%s/%s%s" % (rec[2],rec[1],rec[0])
